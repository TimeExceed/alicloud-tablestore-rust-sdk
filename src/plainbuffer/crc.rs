pub fn crc8_u8(out: &mut u8, inp: u8) {
    const TBL: [u8; 256] = [
    ...
    *out = TBL[(*out ^ inp) as usize];
}

pub fn crc8_u32(out: &mut u8, inp: u32) {
    crc8_u8(out, (inp & 0xFFu32) as u8);
    crc8_u8(out, ((inp >> 8) & 0xFFu32) as u8);
    crc8_u8(out, ((inp >> 16) & 0xFFu32) as u8);
    crc8_u8(out, ((inp >> 24) & 0xFFu32) as u8);
}

pub fn crc8_u64(out: &mut u8, mut inp: u64) {
    for _ in 0..8 {
        let least = (inp & 0xFFu64) as u8;
        inp >>= 8;
        crc8_u8(out, least);
    }
}

pub fn crc8_blob(out: &mut u8, inp: &[u8]) {
    for x in inp.iter() {
        crc8_u8(out, *x);
    }
}
