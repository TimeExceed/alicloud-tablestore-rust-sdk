...
impl Client {
    ...
    pub async fn list_table(&self) -> Result<types::ListTableResponse, Error> {
        ...
        let api = types::ListTable{};
        self.issue(api, req).await
    }

    async fn issue<Api, Req, Resp>(
        &self,
        api: Api,
        req: Req,
    ) -> Result<Resp, Error>
    where
        Api: types::ApiTrait<Request=Req, Response=Resp> + Copy,
        Req: Into<Bytes>,
    ...
    async fn issue_req<Api, Req>(
        &self,
        api: Api,
        req: Req,
    ) -> Result<http::Response<hyper::Body>, Error> 
    where
        Api: types::ApiTrait<Request=Req>,
        Req: Into<Bytes>,
    {
        let url = format!("{}/{}",
    ...
    async fn build_response<Api, Resp>(
        &self, 
        _api: Api,
        resp: http::Response<hyper::Body>,
    ) -> Result<Resp, Error> 
    where
        Api: types::ApiTrait<Response=Resp>,
